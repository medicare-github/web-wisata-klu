...
export const editTourism = (tourism) => {
  return (dispatch, getState, { getFirebase, getFirestore }) => {
    const firestore = getFirestore();
    const idUser = getState().firebase.auth.uid;
    const emailUser = getState().firebase.auth.email;
    ...
    firestore
      ...
      .then(() => {
        dispatch({ type: "UPDATE_TOURISM_SUCCESS", tourism });
      })
      .catch((err) => {
        dispatch({ type: "UPDATE_TOURISM_ERROR", err });
      });
...
export const editCulture = (tourism) => {
  return (dispatch, getState, { getFirebase, getFirestore }) => {
    const firestore = getFirestore();
    const idUser = getState().firebase.auth.uid;
    const emailUser = getState().firebase.auth.email;
...
export const editCulinary = (tourism) => {
  return (dispatch, getState, { getFirebase, getFirestore }) => {
    const firestore = getFirestore();
    const idUser = getState().firebase.auth.uid;
    const emailUser = getState().firebase.auth.email;
